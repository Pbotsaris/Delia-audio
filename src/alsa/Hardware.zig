...
pub fn getCard(self: Hardware, at: usize) !Card {
...
fn loadSystemCards(alsa: *Hardware) !void {
    ...
    while (c_alsa.snd_card_next(&card) >= 0 and card >= 0) {
        ...
        const card_details = try Card.Details.init(alsa.allocator, .{ .card = card, .device = -1 }, c_alsa.snd_ctl_card_info_get_id(info), c_alsa.snd_ctl_card_info_get_name(info));
